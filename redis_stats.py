...
def redis_set(key, value):
    import urllib.parse
    encoded = urllib.parse.quote(json.dumps(value))
    requests.get(f"{REDIS_URL}/set/{key}/{encoded}", headers={"Authorization": f"Bearer {REDIS_TOKEN}"})
...
def update_tg_stats(input_tokens, output_tokens):
    ...
    current = redis_get(month_key) or {"web_cost": 0, "tg_cost": 0, "web_messages": 0, "tg_messages": 0, "whisper_cost": 0, "input_tokens": 0, "output_tokens": 0}
